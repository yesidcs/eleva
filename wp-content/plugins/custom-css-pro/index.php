...
/* Frame loader                                         */
/* ---------------------------------------------------- */
function ccp_frame_loader() {
    
    if(isset($_GET['ccp-iframe']) == true){

        // Be sure, iframe loaded.
        echo "<script>window.loadChecker = 0;

        function ccp_check_load(){

            setTimeout(function(){

                if(typeof parent.ccp_main == 'function'){
                    if(window.loadChecker == 0){
                        parent.ccp_main();
        ...
        window.onload = ccp_check_load();</script>";
        
    }

}
add_action( 'wp_footer', 'ccp_frame_loader', 9999);
    ...
    <body>
        ...
        <div id="ccp-section">
            <div id="ccp-bar">
                ...
                <a class="ccp-btn ccp-visual-editor" target="_blank" href="http://yellowpencil.waspthemes.com/?utm_source=ccp&utm_medium=text&utm_campaign=ccp">Visual Editor [AD]</a>
                <a id="ccp-save" data-nonce="<?php echo wp_create_nonce('ccp_saving'); ?>" class="ccp-btn">Save Changes</a>
            </div>
